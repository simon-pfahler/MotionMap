...
The probability of an edge $e$ being the correct edge for point $p$ is then defined to decay exponentially with $\ell(p,e)$, giving probabilities
%
\begin{align*}
	P(e|p) = \left(\sum_{\text{edges }e'}\exp\big(-\frac{\ell(p,e')}{5\unit{\metre}}\big)\right)^{-1}\exp\big(-\frac{\ell(p,e)}{5\unit{\metre}}\big)
	=Z\exp\big(-\frac{\ell(p,e)}{5\unit{\metre}}\big)\,,
\end{align*}
...
In practice, $Z$ is often hard to calculate, which is why we only work with the likelihood
%
\begin{align*}
	L(p|e) = \exp\big(-\frac{\ell(p|e)}{5\unit{\metre}}\big)\,.
\end{align*}

Using likelihoods, we can obtain likelihood ratios, meaning that for point $p$, edge $e_1$ is $k$-times as likely as edge $e_2$ if
%
\begin{align*}
	\frac{L(p|e_1)}{L(p|e_2)}=k
\end{align*}
...
The likelihood of the track $\mathcal T$ under path $\mathcal P$ is then defined through
%
\begin{align*}
	L(\mathcal P|\mathcal T)=\prod_{i=1}^nL(e_i|p_i)
\end{align*}
